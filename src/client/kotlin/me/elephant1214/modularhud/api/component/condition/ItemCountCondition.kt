 ...
 *
 * @property location Where the item should be fetched from. Cannot be used with [DURABILITY] or [FAKE] and
 * will throw an error when loaded.
 * @property threshold The item count threshold.
 * @property variable The variable for the slot number when using [HOT_BAR] or [ARMOR], or
...
data class ItemCountCondition(
    ...
    override fun compute(handler: ComponentHandler): Boolean {
        return when (this.location) {
            ...
            DURABILITY, FAKE -> error("Shouldn't get here")
        }
    }
}
